...
document.addEventListener('DOMContentLoaded', function () {
  var map = L.map('map').setView([53.5444, -113.4909], 10); // Centered on Edmonton
  ...
  geojson.features.forEach(function (marker) {
    L.marker([marker.geometry.coordinates[1], marker.geometry.coordinates[0]]).bindPopup(marker.properties.title).addTo(map);
  }); // Fit the map to show all markers

  var bounds = L.latLngBounds(geojson.features.map(function (marker) {
